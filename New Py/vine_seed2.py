import pyautogui
import time
import keyboard  # For toggle functionality
...
# Main loop with toggle functionality
def main():
    print("Press 't' to toggle the chest looting script ON/OFF.")
    is_running = False
    ...
    while True:
        if keyboard.is_pressed('t'):
            ...
            if is_running:
                print("Chest looting script activated!")
            else:
                print("Chest looting script deactivated!")
            time.sleep(0.5)  # Prevent multiple toggles from one press

        if is_running:
            loot_chest()
            time.sleep(2)  # Delay between looting runs to avoid spamming
